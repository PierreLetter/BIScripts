...
# REQUIRED FIELDS
$serverName = "db-uat.data.electrical-power.safran"; # name of the instance
$dbName = "DF20_DataFactory"; # name of the database
$sourceFG = "PRIMARY"; # filegroup you're moving objects out of
$destFG = "SECONDARY"; # filegroup you're moving objects into
$logPath = "C:\Users\L005569\Downloads\fg_move_logs"; # all queries are logged to this folder so you have record of what was run

# FILTERS - if these are non-null only the objects that match will be migrated
$schemaFilter = "ref"; # can be $null, a literal "tablename" or a LIKE "%foo%"
...
if ($null -eq $db.FileGroups[$sourceFG]) {
	throw "Source filegroup [$sourceFG] does not exist.";
} 

if ($null -eq $db.FileGroups[$destFG]) {
...
if ($null -ne $minSizeFilter) {
	$qry += "`n`tAND au.data_pages > " + ($minSizeFilter * 128);
}

if ($null -ne $maxSizeFilter) {
...
foreach ($i in $idxList | Sort-Object Size_MB,SchemaName,ObjName) {
	$sn = $i.SchemaName;
	$on = $i.ObjName;
	$iid = $i.IndexID;
	$in = $i.IdxName;
	...
	if($iid -eq 0) {
		...
		foreach($ix in $tbl.Indexes | Where-Object {$_.ID -gt 1}) {
			$j = $ix.ID;
